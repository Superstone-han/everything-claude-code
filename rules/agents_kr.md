# Agent 오케스트레이션

## 사용 가능한 Agent

`~/.claude/agents/`에 위치:

| Agent | 목적 | 사용 시점 |
|-------|---------|-------------|
| planner | 구현 계획 | 복잡한 기능, 리팩토링 |
| architect | 시스템 설계 | 아키텍처 결정 |
| tdd-guide | 테스트 주도 개발 | 새 기능, 버그 수정 |
| code-reviewer | 코드 검토 | 코드 작성 후 |
| security-reviewer | 보안 분석 | 커밋 전 |
| build-error-resolver | 빌드 오류 수정 | 빌드 실패 시 |
| e2e-runner | E2E 테스트 | 중요한 사용자 플로우 |
| refactor-cleaner | 데드 코드 정리 | 코드 유지 관리 |
| doc-updater | 문서화 | 문서 업데이트 |

## 즉시 Agent 사용

사용자 프롬프트 필요 없음:
1. 복잡한 기능 요청 - **planner** agent 사용
2. 코드가 작성/수정됨 - **code-reviewer** agent 사용
3. 버그 수정 또는 새 기능 - **tdd-guide** agent 사용
4. 아키텍처 결정 - **architect** agent 사용

## 병렬 작업 실행

독립적인 작업에는 항상 병렬 Task 실행을 사용하세요:

```markdown
# 좋음: 병렬 실행
3개 agent를 병렬로 실행:
1. Agent 1: auth.ts의 보안 분석
2. Agent 2: 캐시 시스템의 성능 검토
3. Agent 3: utils.ts의 타입 검사

# 나쁨: 불필요한 순차 실행
먼저 agent 1, 그 다음 agent 2, 그 다음 agent 3
```

## 다중 관점 분석

복잡한 문제의 경우 분할 역할 하위 agent 사용:
- 사실 검토자
- 시니어 엔지니어
- 보안 전문가
- 일관성 검토자
- 중복성 검사자
